...
class AssociationManager extends Manager
{
    ...
    public function deleteAssociationBD($id_association)
    {
        $req = "DELETE from association_vehicule_conducteur where id_association = :id_association";
        $statut = $this->getBdd()->prepare($req);
        $statut->bindValue(":id_association", $id_association, PDO::PARAM_INT);
        $result = $statut->execute();
        $statut->closeCursor();
        if ($result) {
            $association = $this->getassociationById($id_association);
            unset($association);
        }
    }
